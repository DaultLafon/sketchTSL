...
export default class Ring2 {
    ...
    anim() {
        this.meshes.forEach((mesh, i) => {
            ...
            gsap.delayedCall(delay, animateStep);
        });
    }
    ...
    update() {
        ...
        if (t > delayStart) {
            const adjustedTime = t - delayStart;
            const speed = stepDistance / stepDuration;
            moveZ = speed * adjustedTime;
        }

        this.experience.camera.instance.position.z = moveZ - 1;
        this.experience.camera.instance.lookAt(new THREE.Vector3(0, 0, moveZ));

        // --- Mise à jour de la vitesse réelle ---
        const deltaTime = this.time.delta * 0.001; // secondes

        this.meshes.forEach((mesh, i) => {
            if (mesh.userData.lastZ !== undefined) {
                const dz = mesh.position.z - mesh.userData.lastZ;
                const speedReal = dz / deltaTime;
                mesh.userData.meshSpeed.value = speedReal;
            }
            mesh.userData.lastZ = mesh.position.z;
